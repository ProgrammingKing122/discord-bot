...
def scoreboard(guild, a, b, ta, tb, stats, winner, notes):
    ...
    ft, fs = font(40), font(22)
    d.text((40, 30), "MATCH RESULTS", font=ft, fill=(255, 255, 255))
    d.text((40, 85), winner, font=fs, fill=(180, 220, 255))
    y = 150
    for name, team in ((a, ta), (b, tb)):
        ...
        for uid in team:
            ...
            d.text((60, y), line, font=fs, fill=(230, 230, 255))
            y += 30
        y += 20
    if notes:
        d.text((40, y+10), "Notes:", font=font(26), fill=(200,200,255))
        d.text((40, y+50), notes[:250], font=fs, fill=(220,220,220))
    buf = BytesIO()
...
class ResultsView(discord.ui.View):
    ...
    def embed(self):
        ...
        def block(team):
            out = []
            for u in team:
                k,d = self.stats.get(u,(0,0))
                out.append(f"{dn(self.guild,u)} • {k}K {d}D KD {kd(k,d)} {dot(k,d)}")
            return "\n".join(out) if out else "—"
        e.add_field(name=f"{self.wager.a} — {self.total(self.wager.ta)} Kills", value=block(self.wager.ta), inline=False)
        e.add_field(name=f"{self.wager.b} — {self.total(self.wager.tb)} Kills", value=block(self.wager.tb), inline=False)
        if self.notes:
...
class WagerView(discord.ui.View):
    ...
    async def mm(self,i,_):
        ...
        v=discord.ui.View()
        async def pick(ii):
            if ii.user.id!=self.host: return
